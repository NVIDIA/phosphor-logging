...
const std::unordered_map<RsyslogFwd::LogType, std::string>
                                            logTypeStringMap = {
...
const std::unordered_map<RsyslogFwd::TransportProtocol, std::string>
    ...
    {RsyslogFwd::TransportProtocol::TCP, "tcp"},
...
const std::unordered_map<RsyslogFwd::NetworkProtocol, std::string>
    ...
    {RsyslogFwd::NetworkProtocol::IPv4, "inet"},
    {RsyslogFwd::NetworkProtocol::IPv6, "inet6"}
};

/* Converts enum values to strings */
template<typename T>
std::string enumToString(T value, const std::unordered_map<T, std::string>& enumStringMap)
{
...
template<typename T>
T stringToEnum(const std::string& str, const std::unordered_map<T, std::string>& enumStringMap)
{
...
void Conf::createRsyslogFwdIndex(size_t index,
                                 ...
                                 uint16_t port)
{
    ...
    if (actionExists(index, logType))
    {
        ...
        elog<InvalidArgument>(Argument::ARGUMENT_NAME("Index"),
...
size_t Conf::createRsyslogFwd(RsyslogFwd::LogType logType,
                              bool enabled,
                              RsyslogFwd::TransportProtocol transportProtocol,
                              RsyslogFwd::NetworkProtocol networkProtocol,
                              std::string address,
                              uint16_t port)
{
    ...
    if (avalIndex == INVALID_INDEX)
    {
        log<level::ERR>("No available index was found for logType",
                        entry("LOG_TYPE=%d", logType));
        elog<InvalidArgument>(Argument::ARGUMENT_NAME("LogType"),
                              Argument::ARGUMENT_VALUE(enumToString(logType,
                                                 logTypeStringMap).c_str()));
...
void Conf::removeRsyslogFwd(size_t index, RsyslogFwd::LogType logType)
{
    ...
    auto it = std::find_if(fwdActions.begin(), fwdActions.end(),
        ...
        { return (fwdActionPtr->index() == index) && (fwdActionPtr->logType() == logType); });

    if (it == fwdActions.end())
    {
        log<level::ERR>("RsyslogFwd was not found",
                        entry("INDEX=%d", index),
                        entry("LOG_TYPE=%d", logType));
        return;
    }
...
bool Conf::addRsyslogFwdObject(size_t index,
                              RsyslogFwd::LogType logType,
                              bool enabled,
                              RsyslogFwd::TransportProtocol transportProtocol,
                              RsyslogFwd::NetworkProtocol networkProtocol,
                              ...
                              uint16_t port)
{
    ...
    {
        log<level::ERR>("Too many entries of logType",
                        entry("COUNT=%d", count),
                        entry("LOG_TYPE=%d", logType));
        elog<InvalidArgument>(Argument::ARGUMENT_NAME("LogType"),
                              Argument::ARGUMENT_VALUE(enumToString(logType,
...
{
    std::string logTypeStr = enumToString(logType, logTypeStringMap);
    std::string filePath = std::string(RSYSLOG_FWD_ACTIONS_CONF_DIR_PATH) + "/fwd_" + logTypeStr + ".conf";
    ...
    configFile << "# Do not edit this file. It is auto-generated by phosphor-rsyslog-config\n\n";
...
bool Conf::createObjectsFromConfigFiles()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator(RSYSLOG_FWD_ACTIONS_CONF_DIR_PATH))
    {
        ...
        if (std::filesystem::is_regular_file(entry.path()) && filenameStr.starts_with("fwd_"))
        {
            ...
            std::string logTypeName = filenameStr.substr(4); /* Removes "fwd_" prefix */
            logTypeName = logTypeName.substr(0, logTypeName.find(".conf")); /* Removes ".conf" suffix */
            RsyslogFwd::LogType logType = stringToEnum(logTypeName, logTypeStringMap);
            ...
            while (std::getline(configFile, line))
            {
                ...
                if (std::regex_match(line, match, actionRegex))
                {
                    ...
                    if (index < MAX_ENTRIES)
                    {
                        addRsyslogFwdObject(index, logType, enabled, transportProtocol, networkProtocol, address, port);
                    }
                }
            }
        }
