...
void selStartup(internal::Manager& logManager)
{
    ...
    for (const auto& entry : logManager.entries)
    {
        ...
        for (const auto& entryItem : entry.second->additionalData())
        {
            try
            {
                if (entryItem.find("SEL_RECORD_ID") != std::string::npos)
                {
                    auto pos = entryItem.find(separator);
                    if (pos != std::string::npos)
                    {
                        lastEntryId = entryId;
                        // Parse the SEL_RECORD_ID value after the separator
                        selRecordId = std::stoul(entryItem.substr(pos + 1));
                        break;
                    }
                }
            }
                ...
                std::vector<std::string>& additionalData)
{
    for (const auto& entryItem : additionalData)
    {
        ...
        if (std::string::npos != found)
        {
            ...
            // SEL_RECORD_ID.
            if ((key == DEFAULT_BIN_KEY) && (val == "SEL"))
            {
                additionalData.emplace_back("SEL_RECORD_ID=" +
