 ...
 * @param[in] dbusPropertyValueList - arguments like dbus property name and value.
 * needs to be in the format of ["propertyName,propertyValue"]
 ...
 * std::vector<std::string> dbusPropertyValueList = {"ModulePowerCap", std::to_string(450)};
 * std::string dbusObjectPath = "/xyz/openbmc_project/inventory/system/processors/GPU_0";
 ...
 * std::string dbusObjectPath = "/xyz/openbmc_project/VirtualMedia/Legacy/USB1";
 ...
 *            Hereonly propertyValue is required therefore dbusPropertyValueList is having
 * only propertyValue
 * std::vector<std::string> dbusPropertyValueList {"FactoryReset"};
 * std::string ooc = "/xyz/openbmc_project/state/bmc0/Mananger.ResetToDefaults";
...
/**
 * @brief dbus log when resource created/modified/deleted.
 * This API specially for components which don't have the sdbusplus::asio::connection object.
 * @param[in] message  - Message enums RESOURCE_CREATED, RESOURCE_DELETED,
 * PROPERTY_VALUE_MODIFIED etc.
 * @param[in] severity - serverity level
 * @param[in] dbusPropertyValueList - arguments like dbus property name and value.
